...
public class PointAdjustmentTable {
    public void truncateData() throws SQLException {
        ...
        PreparedStatement ps = connection.prepareStatement("TRUNCATE `point_adjustment`");
        ps.execute();
        ps.close();
        connection.close();
    }
    ...
    public void addAdjustmentData(List<ResultRisk> resultRisks) throws SQLException {
        ...
        PreparedStatement ps = connection.prepareStatement("INSERT INTO `point_adjustment` (`existing_file_id`, `loan_id`, `index`, `change`) VALUES (?, ?, ?, ?)");
